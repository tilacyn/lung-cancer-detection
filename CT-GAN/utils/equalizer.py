...
class histEq:
    def __init__(self, X_sample, levels=10000, nbins=10000,
                 path=[]):  # X_sample is a list of one or more images sampled from the distribution, levels is the ourput norm range
        if len(path) > 0:
            self.load(path)
        else:
            self.nbins = nbins
